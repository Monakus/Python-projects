...
class Board:
    ...
    def _is_valid(self, t, num):
        return self._check_row(t, num) and self._check_column(t, num) and self._check_box(t, num)

    def _check_row(self, t, num):
        row = t.pos[0]
    ...
    def _check_column(self, t, num):
        col = t.pos[1]
        for i in range(9):
            if self.tiles[i][col].val == num:
                return False
        return True

    def _check_box(self, t, num):
        row = t.pos[0]
        col = t.pos[1]
        box_row, box_col = row - row % 3, col - col % 3
        for i in range(3):
            for j in range(3):
                if self.tiles[box_row + i][box_col + j].val == num:
                    return False
        return True
